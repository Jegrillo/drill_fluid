...
def sns(x, y):
    ...
    density_drill_mud = x
    density_drill_mud = density_drill_mud / 1000  # При расчете используется плотность в г/см3
    plastic_vis = y
    ...
    bit_diameter = float(input("Введите диаметр долота, мм: "))
    bit_diameter = bit_diameter / 10  # При расчете используется диаметр в см
    ...
    density_rock = density_rock / 1000  # При расчете используется плотность в г/см3
    ...
    column_diameter_outer = column_diameter_outer / 1000  # При расчете используется диаметр в м
    column_wall_thickness = float(input("Введите толщину стенки обсадной трубы (9, если не указано иной), мм: "))
    column_wall_thickness = column_wall_thickness / 1000  # При расчете используется толщина в м
    ...
    volume_rock = 0.785 * koef_kav * ((bit_diameter / 100) ** 2) * (depth_drilling - depth_casing)
    ...
    clean_num_stage = input("Введите количество ступеней очистки (I - IV): ")
    ...
    if choise == 'Y' or choise == 'y':
        ...
        density_weight = density_weight / 1000  # При расчете используется плотность в г/см3
        density_initial = float(input("Введите плотность бурового раствор до обработки утяжелителем, кг/м3: "))
        density_initial = density_initial / 1000  # При расчете используется плотность в г/см3
    ...
    output = []
    output.append(sns_60)
    output.append(sns_600)
    return output
